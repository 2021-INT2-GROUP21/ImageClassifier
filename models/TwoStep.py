...
class TwoStep(nn.Module):
    def __init__(self):
        ...
        self.conv_layer0 = nn.Sequential(
            ...
            nn.Conv2d(in_channels=2 * m, out_channels=3 * m, kernel_size=(5, 5), padding=(2, 2)),
            nn.BatchNorm2d(3 * m),
            nn.ReLU(inplace=True),

            nn.Conv2d(in_channels=3 * m, out_channels=4 * m, kernel_size=(13, 13), padding=(6, 6)),
            nn.BatchNorm2d(4 * m),
            nn.ReLU(inplace=True),
